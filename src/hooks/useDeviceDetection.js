...
const useDeviceDetection = () => {
  ...
  useEffect(() => {
    ...
    const checkDevice = () => {
      ...
      const mobileKeywords = ['android', 'iphone', 'ipod', 'opera mini', 'windows phone'];
      const tabletKeywords = ['ipad', 'tablet'];
      
      // Check if mobile
      const checkMobile = mobileKeywords.some(keyword => userAgent.includes(keyword));
      
      // Check if tablet
      const checkTablet = tabletKeywords.some(keyword => userAgent.includes(keyword));
      
      // Set states based on checks
      setIsMobile(checkMobile && !checkTablet);
    ...
    const checkOrientation = () => {
      if (window.matchMedia) {
        ...
        setIsLandscape(isLandscapeQuery.matches);
        
        // Add listener for orientation changes
        const handleOrientationChange = (e) => setIsLandscape(e.matches);
        isLandscapeQuery.addEventListener('change', handleOrientationChange);
        
        // Clean up
        return () => {
          isLandscapeQuery.removeEventListener('change', handleOrientationChange);
        };
      } else {
